...
public class GenerateBone {
    ...
    public static void apply_posture(Quaternion[] rot, Transform[] body, bool generate_bone=false )
    {
        if (generate_bone)
        {
            body[GenerateBone.HIP].parent.localRotation = rot[GenerateBone.HIP];
    ...
    public static GameObject generate_bone(Vector3 [] pos, float scale, out Transform[] body, bool generate_spheer=false)
    {
        ...
        body_base.transform.localPosition = pos[GenerateBone.HIP];
        ...
        body[GenerateBone.HIP].localPosition = new Vector3(0, 0, 0);
        body[GenerateBone.HIP].localRotation = Quaternion.identity;
